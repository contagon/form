...
class OxfordSpiresCustom(Dataset):
    ...
    def cam_T_lidar(self) -> SE3:
        r = SO3(
            qx=0.5034725628949727,
            qy=0.5003337192107279,
            qz=-0.4947539264483171,
            qw=0.5013981452864594,
        )
        t = np.array(
            [
    ...
    def cam_T_imu(self) -> SE3:
        r = SO3(
            qx=0.0042909719518877815,
            qy=-0.7085293318962699,
            qz=0.705668106144218,
            qw=0.0005453193252705428,
        )
        t = np.array(
            [
                -0.0008544600980918986,
                0.005115741648037186,
                -0.06898747851173177,
            ]
        )
        return SE3(r, t)
    ...
    def imu_params(self) -> ImuParams:
        # Params from their google drive imu.yaml file
        return ImuParams(
            gyro=0.00022632861789099884,
            accel=0.0011002607647952406,
            gyro_bias=8.252445860125436e-06,
            accel_bias=3.390710627779767e-05,
            bias_init=1e-8,
            integration=1e-8,
