/*
    nanobind/stl/shared_ptr.h: Type caster for boost::shared_ptr<T>
 ...
 */
template <typename T>
inline NB_NOINLINE boost::shared_ptr<T> shared_from_python_boost(T *ptr,
                                                                 handle h) noexcept {
  if (ptr)
    return boost::shared_ptr<T>(ptr, py_deleter_boost{h.inc_ref().ptr()});
  else
    return boost::shared_ptr<T>(nullptr);
}

inline NB_NOINLINE void shared_from_cpp_boost(boost::shared_ptr<void> &&ptr,
                                              PyObject *o) noexcept {
  keep_alive(o, new boost::shared_ptr<void>(std::move(ptr)),
             [](void *p) noexcept { delete (boost::shared_ptr<void> *)p; });
}

template <typename T> struct type_caster<boost::shared_ptr<T>> {
  static constexpr bool IsClass = true;
  using Caster = make_caster<T>;
  using Td = std::decay_t<T>;

  NB_TYPE_CASTER(boost::shared_ptr<T>, Caster::Name)
  ...
  bool from_python(handle src, uint8_t flags, cleanup_list *cleanup) noexcept {
    ...
    if constexpr (has_shared_from_this_v<T>) {
      ...
      // Otherwise create a new one. Use shared_from_python_boost<T>(...)
      // so that future calls to ptr->shared_from_this() can share
      // ownership with it.
      value = shared_from_python_boost(ptr, src);
    } else {
      value = boost::static_pointer_cast<T>(
          shared_from_python_boost(static_cast<void *>(ptr), src));
    }
    return true;
  }
  ...
  static handle from_cpp(const Value &value, rv_policy,
    ...
    if constexpr (!std::is_polymorphic_v<Td>) {
      ...
      result =
          nb_type_put_p(type, type_p, ptr, rv_policy::reference, cleanup, &is_new);
    }

    if (is_new) {
      boost::shared_ptr<void> pp;
      if constexpr (std::is_const_v<T>)
        pp = boost::static_pointer_cast<void>(boost::const_pointer_cast<Td>(value));
      else
        pp = boost::static_pointer_cast<void>(value);
      shared_from_cpp_boost(std::move(pp), result.ptr());
    }
